...
public class DocumentDBAccess implements DocumentDataAccess{
    ...
    public void createDocument(DocumentType type, int documentNumber, Date date, float priceVATExcluded, 
        ...
        try {
            ...
            preparedStatement.setInt(7, workflow.getId());
            preparedStatement.setString(8, status.getCode());
    ...
    public ArrayList<Document> readDocumentsOfAWorkflow(Workflow workflowWanted){
        String sqlInstruction = "select * from document where workflowId = ?";
        Connection connection = SingletonConnexion.getInstance();
        ArrayList<Document> allDocuments = new ArrayList<>();
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(sqlInstruction);
            preparedStatement.setInt(1, workflowWanted.getId());
            ResultSet data = preparedStatement.executeQuery();
            Document document;
            while(data.next()){
                document = new Document(type.readOneDocumentType(data.getString("type")), 
    ...
    public ArrayList<Document> readDocumentsOfAStatus(DocumentStatus statusWanted){
        String sqlInstruction = "select * from document where status = ?";
        Connection connection = SingletonConnexion.getInstance();
        ArrayList<Document> allDocuments = new ArrayList<>();
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(sqlInstruction);
            preparedStatement.setString(1, statusWanted.getCode());
            ResultSet data = preparedStatement.executeQuery();
            Document document;
            while(data.next()){
                document = new Document(type.readOneDocumentType(data.getString("type")), 
    ...
    public ArrayList<Document> readDocumentsOfAType(DocumentType typeWanted){
        String sqlInstruction = "select * from document where type = ?";
        Connection connection = SingletonConnexion.getInstance();
        ArrayList<Document> allDocuments = new ArrayList<>();
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(sqlInstruction);
            preparedStatement.setString(1, typeWanted.getCode());
            ResultSet data = preparedStatement.executeQuery();
            Document document;
            while(data.next()){
                document = new Document(type.readOneDocumentType(data.getString("type")), 
    ...
    public Document readOneDocument(DocumentType typeWanted, int numberWanted){
